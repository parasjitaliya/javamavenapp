...
public class EasyPointRefund {
	public String data="RevdemoAPIExcels/EasyPointRefund.xls";
	...
	File f;
	String JSONtag; 	
	public ExtentReports report=new ExtentReports("ExtentReports/Revdemo/EasyPointRefund.html");
	public ExtentTest logger =report.startTest("EasyPointRefund");
	DateFormat df=new SimpleDateFormat("dd MMM YYYY");
	Date d=new Date();
	String time=df.format(d); 
  @Test
  public void wsEasyPointRefund() throws IOException, BiffException {
	  f=new File(data);
	  wb = Workbook.getWorkbook(f);
	  s=wb.getSheet(0);
	  System.setProperty("webdriver.chrome.driver","chromedriverchromedriverchromedriverchromedriverchromedriver");
	  ...
	  dropdown.selectByVisibleText("wsEasyPointRefund");
	  ...
	  a.sendKeys(Keys.INSERT,",").build().perform();
	  a.sendKeys(Keys.ENTER,"\"RefundDate\":").build().perform();
	  a.sendKeys(Keys.INSERT,"\""+time+"\"").build().perform();
	  a.sendKeys(Keys.INSERT,",").build().perform();
	  a.sendKeys(Keys.ENTER,"\"RefundTransactionId\":").build().perform();
	  a.sendKeys(Keys.INSERT,"\""+s.getCell(5,i).getContents()+"\"").build().perform();
	  a.sendKeys(Keys.INSERT,",").build().perform();
	  a.sendKeys(Keys.ENTER,"\"RefundAmount\":").build().perform();
	  a.sendKeys(Keys.INSERT,"\""+s.getCell(6,i).getContents()+"\"").build().perform();
	  a.sendKeys(Keys.INSERT,",").build().perform();
	  a.sendKeys(Keys.ENTER,"\"RefundEasyPoints\":").build().perform();
	  ...
	  FileUtils.copyFile(src, new File("screenshots/Revdemo/EasyPointRefund."+i+".png"));
	  String JSONresponse=driver.findElement(By.xpath("//*[@id='txtResponse']")).getText();
		System.out.println(JSONresponse);
		
	  if(JSONresponse.contains("Success")){
			    System.out.println("Pass");
		    	logger.log(LogStatus.PASS,"Response is Success",logger.addScreenCapture("screenshots/Revdemo/EasyPointRefund."+i+".png")); 
	  }
	  else{
			    System.out.println("Fail");
		    	logger.log(LogStatus.FAIL ,"Failed",logger.addScreenCapture("screenshots/Revdemo/EasyPointRefund."+i+".png")); 
	  } 
